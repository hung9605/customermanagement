...
public interface MedicalExaminationRepository extends JpaRepository<MedicalExamination,Integer> {
	...
	@Query("select new com.app.customermanagement.dto.model.MoneyDto(se.id,se.fullName,se.dateRegister,me.money,me.status) from MedicalExamination me " +
			"inner join ScheduleMedical se on me.medical.id = se.id where me.dayOfExamination = :date")
	List<MoneyDto> listMoney(String date);
