...
public interface InventoryRepository extends JpaRepository<Inventory, Integer>{
	...
	@Query(value = """
		WITH inventory_detail_filtered AS (
		    SELECT
		        i.id,
		...
		combined AS (
		    SELECT * FROM inventory_detail_filtered
		    UNION ALL
		...
		ORDER BY medical_supplies_id, status, row_order;
		    """, nativeQuery = true)
	List<InventoryReportDTO> getInventoryReport(@Param("fromDate") String fromDate, @Param("toDate") String toDate);
